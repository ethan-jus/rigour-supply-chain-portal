  ...
  >
    <div v-loading="loading">
      ...
      <template v-if="context">
        ...
        >
          ...
          <el-table-column label="行" width="50"
            ><template #default="{ row }">{{ row.original.lineNo }}</template></el-table-column
          >
          <el-table-column label="原商品 / 规格" min-width="200">
            <template #default="{ row }"
              >{{ row.original.productNameSnapshot || '-' }} /
              {{ row.original.specificationSnapshot || '-' }}</template
            >
          </el-table-column>
          ...
          >
          <el-table-column label="记录单位" width="110"
            ><template #default="{ row }">{{
              unitLabel(row.original.unitCode)
            }}</template></el-table-column
          >
          <el-table-column label="成交单价" width="110"
            ><template #default="{ row }">{{
          ...
          <el-table-column label="当前复核" min-width="200">
            <template #default="{ row }">
              ...
              <template v-else-if="row.repair">
                <div>
                  {{ row.repair.lines[0]?.proposed?.productName }} /
                  {{ row.repair.lines[0]?.proposed?.specification || '默认规格' }}
                </div>
                <div>
                  历史单位：{{
                    unitLabel(
                      row.repair.lines[0]?.requested?.historicalTransactionUnitCode,
                      '未核实',
                    )
                  }}
                </div>
                <div v-if="row.repair.lines[0]?.requested?.standardQuantity != null">
                  标准数量：{{ number(row.repair.lines[0].requested.standardQuantity) }}
                  {{ unitLabel(row.repair.lines[0].requested.standardUnitCode) }}
                </div>
              </template>
        ...
        >
          ...
          <section v-for="line in selected" :key="line.original.id" class="repair-line-form">
            <h3>
              第 {{ line.original.lineNo }} 行 ·
              {{ line.original.productNameSnapshot || '商品待确认' }}
            </h3>
            <div class="repair-grid">
              <el-form-item label="ERP 商品">
                <el-select
                  v-model="draft(line).selectedProductId"
                  filterable
                  remote
                  clearable
              ...
              <el-form-item label="ERP 规格">
                <el-select
                  v-model="draft(line).skuCode"
                  :disabled="!draft(line).selectedProductId || productSelecting"
                  clearable
                >
                  <el-option
                    v-for="variant in variants[String(line.original.id)] || []"
                    :key="variant.id"
              ...
              <el-form-item label="商品对应复核依据" required class="repair-full">
                <el-input
                  v-model="draft(line).bindingEvidence"
                  type="textarea"
                  :rows="2"
                  maxlength="2000"
                />
              ...
              <el-collapse v-if="draft(line).sourceNamespace" class="repair-full">
                <el-collapse-item title="来源商品对应证据（选填）" :name="line.original.id">
                  ...
                  <div class="repair-grid">
                    <el-form-item label="来源商品编码"
                      ><el-input v-model="draft(line).sourceProductCode" maxlength="200" readonly
                    /></el-form-item>
                    <el-form-item label="来源对应凭证依据" class="repair-full"
                      ><el-input v-model="draft(line).sourceEvidence" maxlength="2000"
                    /></el-form-item>
                    <el-checkbox v-model="draft(line).confirmSourceIdentity" class="repair-full"
                      >已人工核对本笔交易对应的来源商品</el-checkbox
                    >
                  </div>
                </el-collapse-item>
              </el-collapse>
              ...
              <el-checkbox
                v-model="draft(line).confirmHistoricalTransactionUnit"
                class="repair-full"
              >
                已核对历史交易单位凭证
              </el-checkbox>
              <el-checkbox v-model="draft(line).includeStandard" class="repair-full"
                >记录经复核的标准计量</el-checkbox
              >
              <template v-if="draft(line).includeStandard">
                <el-form-item label="标准数量" required
                  ><el-input v-model="draft(line).standardQuantity" inputmode="decimal"
                /></el-form-item>
                ...
                <el-form-item label="历史换算系数" required
                  ><el-input v-model="draft(line).conversionFactor" inputmode="decimal"
                /></el-form-item>
                <el-form-item label="历史换算证据" required
                  ><el-input v-model="draft(line).conversionEvidence" maxlength="2000"
                /></el-form-item>
                <el-checkbox v-model="draft(line).confirmHistoricalConversion" class="repair-full"
                  >已核实历史换算关系与标准数量</el-checkbox
                >
              </template>
            </div>
          </section>
        ...
        <section v-if="preview" class="repair-preview" data-testid="repair-preview">
          <div class="repair-heading">
            <strong>修复预览</strong>
            <el-tag :type="preview.status === 'READY' ? 'success' : 'danger'">{{
              preview.status === 'READY' ? '待确认' : '存在阻断'
            }}</el-tag>
            <span>有效至 {{ displayDateTime(preview.expiresAt) }}</span>
          </div>
          <div v-for="line in preview.lines" :key="line.original.id" class="repair-preview-line">
            <strong>第 {{ line.original.lineNo }} 行</strong>
            <div v-for="blocker in line.blockers" :key="blocker" class="repair-blocker">
              {{ blocker }}
            </div>
            <div v-if="line.proposed">
              {{ line.original.productNameSnapshot || '原商品待补齐' }} /
              {{ line.original.specificationSnapshot || '-' }} → {{ line.proposed.productName }} ·
              {{ line.proposed.specification || '默认规格' }}
            </div>
            <div v-else-if="line.candidates.length">
              候选：{{
                line.candidates
                  .map((c) => `${c.productName} / ${c.specification || '默认规格'}`)
                  .join('；')
              }}
            </div>
            <div>
              原数量 {{ number(line.original.quantity) }} · 记录单位
              {{ unitLabel(line.original.unitCode) }} · 历史交易单位
              {{ unitLabel(line.requested.historicalTransactionUnitCode, '未核实') }}
            </div>
            <div v-if="line.requested.standardQuantity != null">
              标准计量 {{ number(line.requested.standardQuantity) }}
              {{ unitLabel(line.requested.standardUnitCode) }} · 换算系数
              {{ number(line.requested.conversionFactor) }}
            </div>
            <div>
              成交单价 {{ money(line.original.unitPrice) }} · 折扣
              {{ money(line.original.discountAmount) }} · 应收 {{ money(line.original.lineAmount) }}
            </div>
            <div>商品依据：{{ line.requested.bindingEvidence || '-' }}</div>
            <div v-if="line.requested.transactionUnitEvidence">
              单位凭证：{{ line.requested.transactionUnitEvidence }}
            </div>
            <div v-if="line.requested.conversionEvidence">
              换算凭证：{{ line.requested.conversionEvidence }}
            </div>
            <div v-if="line.sourceIdentityStatus === 'OPERATOR_CONFIRMED'">
              来源商品人工确认：{{ line.requested.sourceProductCode || '来源商品记录' }} ·
              {{ line.requested.sourceEvidence }}
            </div>
          </div>
          <el-checkbox
            v-model="confirmed"
            :disabled="preview.status !== 'READY' || applying"
        ...
        <el-collapse v-if="context.recentRepairs.length" class="repair-history">
          <el-collapse-item title="最近修复记录（最多50次）" name="history">
            <div
              v-for="record in context.recentRepairs"
              :key="record.previewId"
              class="repair-preview-line"
            >
              <div>{{ displayDateTime(record.appliedAt) }} · 人工复核</div>
              <div v-for="line in record.lines" :key="line.original.id">
                第 {{ line.original.lineNo }} 行 · {{ line.proposed?.productName }} /
                {{ line.proposed?.specification || '默认规格' }} ·
                {{ line.requested.bindingEvidence }}
                <div v-if="line.requested.transactionUnitEvidence">
                  单位凭证：{{ line.requested.transactionUnitEvidence }}
                </div>
                <div v-if="line.requested.conversionEvidence">
    ...
    <template #footer>
      ...
      <el-button
        :icon="View"
        :loading="previewing"
        :disabled="!selected.length || !reason.trim() || applying || loading || productSelecting"
        @click="createPreview"
        >生成预览</el-button
      >
      <el-button
...
import {
  applyOrderProductRepair,
  getOrderProductRepair,
  previewOrderProductRepair,
  getRepairProduct,
  getRepairProducts,
  type RepairContext,
  type RepairLineCommand,
  type RepairPreview,
...
const canApply = computed(
  () =>
    preview.value?.status === 'READY' && confirmed.value && !previewing.value && !applying.value,
)

watch(
  [reason, selected, drafts],
  () => {
    generation++
    preview.value = null
...
watch(
  () => [props.modelValue, props.orderId, props.sourceContext],
  () => {
    generation++
    loadGeneration++
    preview.value = null
...
async function load() {
  ...
  try {
    ...
    for (const line of result.lines) {
      ...
      const applies =
        source &&
        (!source.sourceOrderNo || source.sourceOrderNo === result.sourceOrderNo) &&
        (source.lineId ? source.lineId === String(line.original.id) : result.lines.length === 1)
      drafts[String(line.original.id)] = {
        lineId: line.original.id,
        includeStandard: false,
        ...(applies
          ? {
              sourceNamespace: source.sourceNamespace,
              sourceCaptureRef: source.sourceCaptureRef,
              sourceProductRecordId: source.sourceProductRecordId,
...
function draft(line: SourceLine): Draft {
  return drafts[String(line.original.id)]!
}
function selectable(line: SourceLine) {
  return Number(line.original.quantity) > 0
}
function selectLines(lines: SourceLine[]) {
  ...
  if (
    ...
    source.lineId ||
    lines.length !== 1 ||
    source.sourceOrderNo !== context.value?.sourceOrderNo
  )
    return
...
async function searchProducts(keyword: string) {
  ...
  try {
    ...
    if (current === searchGeneration) productLoading.value = false
  }
}
...
function command(line: SourceLine): RepairLineCommand {
  ...
  return {
    ...
    skuCode: d.skuCode?.trim() || undefined,
    bindingEvidence: d.bindingEvidence?.trim() || undefined,
    ...(reviewSource
      ? {
          sourceNamespace: d.sourceNamespace?.trim() || undefined,
          sourceCaptureRef: d.sourceCaptureRef?.trim() || undefined,
          sourceProductRecordId: d.sourceProductRecordId?.trim() || undefined,
          sourceProductCode: d.sourceProductCode?.trim() || undefined,
          sourceEvidence: d.sourceEvidence?.trim() || undefined,
          confirmSourceIdentity: d.confirmSourceIdentity === true,
        }
      : {}),
    ...
    ...(d.includeStandard
      ? {
          ...
          standardUnitCode: d.standardUnitCode?.trim() || undefined,
          conversionFactor: d.conversionFactor,
          conversionEvidence: d.conversionEvidence?.trim() || undefined,
...
async function createPreview() {
  ...
  try {
    const result = await previewOrderProductRepair(context.value.orderId, {
      ...
      lines: selected.value.map(command),
    })
