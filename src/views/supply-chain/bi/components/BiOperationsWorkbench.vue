...
const amountText = (target: BiTarget) =>
  Number(target.targetValue).toLocaleString('zh-CN', {
    minimumFractionDigits: target.metricCode.endsWith('_CUSTOMER') ? 0 : 2,
...
async function reload() {
  ...
  try {
    if (tab.value === 'targets') {
      const result = await getBiTargets({
        month: month.value,
        dimensionType: dimension.value,
        dimensionCode: dimensionCode.value || undefined,
      })
...
async function submitTarget() {
  ...
  if (!selected && !targetRecord.value) {
    detailError.value = '请选择目标对象'
    return
  }
  if (!/^\d{1,12}(\.\d{1,2})?$/.test(value.value.trim())) {
    detailError.value = '目标值需为非负数，最多两位小数'
    return
  }
  saving.value = true
  detailError.value = ''
...
async function removeTarget(target: BiTarget) {
  try {
    await ElMessageBox.confirm(
      `删除「${target.dimensionName} · ${metrics[target.metricCode]}」目标？`,
      '删除目标',
      { type: 'warning' },
    )
  } catch {
  ...
  <section class="bi-operations" aria-label="经营目标与跟进">
    ...
    <template v-else>
      ...
      <div v-if="tab === 'targets'" class="work-area" :aria-busy="busy">
        ...
        <p v-if="!canTargetWrite" class="permission-note" role="status">
          当前账号仅可查看经营目标；维护目标需管理员授予相应权限。
        </p>
        <form
          v-if="targetEditing && canTargetWrite"
          class="editor"
          aria-label="编辑经营目标"
        ...
        >
          ...
          <label
            >{{ metric.endsWith('_CUSTOMER') ? '目标值（个）' : '目标值（元）' }}
            <el-input
              v-model="value"
              inputmode="decimal"
              aria-label="目标值"
          ...
          <div class="editor-actions wide">
            <el-button native-type="submit" type="primary" :icon="Check" :loading="saving"
              >保存目标</el-button
            >
        ...
        <el-table :data="targets" empty-text="当前月份尚未配置目标" aria-label="经营目标列表">
          ...
          <el-table-column label="指标" min-width="100"
            ><template #default="{ row }">{{
              metrics[row.metricCode as BiTargetMetric]
            }}</template></el-table-column
          >
          <!-- @vue-generic {BiTarget} -->
          <el-table-column label="目标值" min-width="130" align="right"
            ><template #default="{ row }"
              >{{ amountText(row) }}
              {{ row.metricCode.endsWith('_CUSTOMER') ? '个' : '元' }}</template
            ></el-table-column
          >
          <el-table-column prop="remark" label="备注" min-width="140" show-overflow-tooltip />
          <!-- @vue-generic {BiTarget} -->
          <el-table-column v-if="canTargetWrite" label="操作" width="96"
            ><template #default="{ row }">
