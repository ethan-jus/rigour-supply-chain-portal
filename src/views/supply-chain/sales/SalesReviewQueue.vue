    ...
    <template v-else>
        ...
        >
          ...
          <el-table-column prop="dwellMinutes" label="停留" width="90" sortable>
            <template #default="scope">
              <span
                :class="{
                  'evidence-warning': scope.row.dwellMinutes < scope.row.minimumDwellMinutes,
                }"
              >
            ...
            <template #default="scope">
              ...
              <div v-else class="evidence-cell">
                <span
                  :class="{
                    'evidence-warning':
                      scope.row.verifiedRecordingSeconds < scope.row.minimumRecordingSeconds,
                  }"
                >
                  已核验 {{ formatSeconds(scope.row.verifiedRecordingSeconds) }} /
                  {{ formatSeconds(scope.row.minimumRecordingSeconds) }}
                </span>
          ...
          <el-table-column label="门头照" width="110">
            <template #default="scope">
              <span
                :class="{
                  'evidence-warning':
                    scope.row.verifiedStorefrontPhotoCount < scope.row.requiredStorefrontPhotoCount,
                }"
              >
                {{ scope.row.verifiedStorefrontPhotoCount }} /
                {{ scope.row.requiredStorefrontPhotoCount }} 张
              </span>
            </template>
          </el-table-column>
          <el-table-column label="异常原因" min-width="190">
            <template #default="scope">
              <div v-if="scope.row.anomalyCodes.length" class="anomaly-list">
                <el-tag
                  v-for="code in scope.row.anomalyCodes"
                  :key="code"
                  type="warning"
                  size="small"
                >
          ...
          <el-table-column label="拜访结果" min-width="180">
            <template #default="scope">
              <div class="result-cell">
                ...
                <span v-if="scope.row.contactOutcome === 'CONTACTED'"
                  >KP：{{ scope.row.kpName || '未填写' }} · 意向：{{
                    intentionLabel(scope.row.intentionLevel)
                  }}</span
                >
      ...
      <el-dialog v-model="reviewDialog" title="拜访有效性复核" width="520px" destroy-on-close>
        <div v-if="selectedVisit" class="review-context">
          ...
          <span
            >{{ selectedVisit.salesNo }} · 停留 {{ selectedVisit.dwellMinutes }}/{{
              selectedVisit.minimumDwellMinutes
            }}
            分钟 · 核验录音 {{ formatSeconds(selectedVisit.verifiedRecordingSeconds) }}/{{
              formatSeconds(selectedVisit.minimumRecordingSeconds)
            }}
            · 门头照 {{ selectedVisit.verifiedStorefrontPhotoCount }}/{{
              selectedVisit.requiredStorefrontPhotoCount
            }}
            张</span
          >
          <span
            >{{ contactOutcomeLabel(selectedVisit.contactOutcome)
            }}<template v-if="selectedVisit.contactOutcome === 'CONTACTED'">
              · KP {{ selectedVisit.kpName || '未填写' }} ·
              {{ intentionLabel(selectedVisit.intentionLevel) }}</template
            ></span
          >
          <div v-if="selectedVisit.anomalyCodes.length" class="anomaly-list">
            <el-tag
              v-for="code in selectedVisit.anomalyCodes"
              :key="code"
              type="warning"
              size="small"
              >{{ anomalyLabel(code) }}</el-tag
            >
          </div>
        ...
        <section v-if="canViewEvidence" class="evidence-review">
          <div class="evidence-review__heading">
            <strong>现场门头照</strong>
            <span v-if="evidence"
              >已核验 {{ evidence.verifiedStorefrontPhotoCount }} /
              {{ evidence.requiredStorefrontPhotoCount }} 张</span
            >
          </div>
          <div v-if="evidenceLoading" class="recording-review__state">
            <el-icon class="is-loading"><Loading /></el-icon> 正在读取照片证据
          </div>
          <el-alert
          ...
          <div v-else-if="evidence?.photos.length" class="photo-list">
            <div v-for="photo in evidence.photos" :key="photo.evidenceId" class="photo-item">
              <img
                v-if="photoUrls[photo.evidenceId]"
                :src="photoUrls[photo.evidenceId]"
                alt="现场门头照"
              />
              <el-button
                v-else
                plain
                :loading="loadingPhotoId === photo.evidenceId"
                @click="loadPhoto(photo)"
                >授权查看</el-button
              >
              <div>
                <strong>{{
                  photo.evidenceStatus === 'TECHNICALLY_VERIFIED'
                    ? '技术校验通过'
                    : photo.evidenceStatus
                }}</strong>
                <span
                  >距门店约 {{ Math.round(photo.distanceToTargetMeters || 0) }} 米 ·
                  {{ formatDateTime(photo.capturedAt) }}</span
                >
              </div>
            </div>
          </div>
          ...
          <small
            >照片只在具备敏感证据权限后加载，每次查看都会记录审计；是否真正包含门头仍需主管判断。</small
          >
        </section>
        <el-alert
          v-else
        ...
        <section v-if="canPlayRecordings" class="recording-review">
          <div class="recording-review__heading">
            ...
            <span v-if="recordings"
              >{{ recordings.clipCount }} 段 ·
              {{ formatSeconds(Math.round(recordings.uploadedTotalDurationMs / 1000)) }}</span
            >
          </div>
          <div v-if="recordingsLoading" class="recording-review__state">
            <el-icon class="is-loading"><Loading /></el-icon> 正在读取录音
          </div>
          <el-alert
          ...
          <div v-else-if="recordings?.clips.length" class="recording-list">
            <div v-for="clip in recordings.clips" :key="clip.clipId" class="recording-item">
              <div>
                <strong>第 {{ clip.clipIndex + 1 }} 段</strong
                ><span
                  >{{ formatSeconds(Math.round((clip.clientDurationMs || 0) / 1000)) }} ·
                  {{ formatBytes(clip.objectSizeBytes) }}</span
                >
              </div>
              <audio
                v-if="clipUrls[clip.clipId]"
                :src="clipUrls[clip.clipId]"
                controls
                preload="metadata"
              />
              <el-button
                v-else
                plain
                :loading="loadingClipId === clip.clipId"
                @click="loadClip(clip)"
                >授权播放</el-button
              >
            </div>
...
function anomalyLabel(value: string): string {
  const labels: Record<string, string> = {
    VISIT_TIME_INVALID: '到离店时间异常',
    RESULT_MISSING: '拜访结果缺失',
    CONTACT_NOT_CONFIRMED: '未实际接触 KP',
    DWELL_TOO_SHORT: '停留时长不足',
    STOREFRONT_PHOTO_MISSING: '门头照缺失',
    RECORDING_UNVERIFIED: '录音未通过服务端核验',
    RECORDING_TOO_SHORT: '有效录音时长不足',
    AI_PENDING: 'AI 分析未完成',
    AI_REVIEW_REQUIRED: 'AI 建议人工复核',
...
async function loadEvidence(visitId: string) {
  ...
  evidenceError.value = ''
  try {
    evidence.value = (await apiClient.get(
      `/sales/management/visits/${visitId}/evidence`,
    )) as ManagementVisitEvidence
  } catch (error) {
...
async function loadRecordings(visitId: string) {
  ...
  recordingsError.value = ''
  try {
    recordings.value = (await apiClient.get(
      `/sales/management/visits/${visitId}/recordings`,
    )) as ManagementRecordingSession
  } catch (error) {
...
onMounted(() => {
  if (canReview.value) void load()
})
onBeforeUnmount(() => {
  revokeClipUrls()
  revokePhotoUrls()
})
watch(reviewDialog, (visible) => {
  if (!visible) {
    revokeClipUrls()
    revokePhotoUrls()
  }
...
.recording-review__heading,
.evidence-review__heading {
  display: flex;
  align-items: center;
  justify-content: space-between;
}
.recording-review__heading span,
.evidence-review__heading span,
.recording-review > small,
.evidence-review > small {
  color: $color-text-secondary;
  font-size: $font-size-xs;
}
.recording-review__state {
  color: $color-text-secondary;
  font-size: $font-size-sm;
}
.photo-list {
...
.recording-item {
  display: grid;
  grid-template-columns: minmax(110px, 0.6fr) minmax(190px, 1.4fr);
  gap: $spacing-sm;
  align-items: center;
  padding: 10px;
  background: $color-bg-muted;
...
@media (max-width: 760px) {
  .review-toolbar :deep(.el-date-editor) {
    width: 100%;
  }
  .pagination {
