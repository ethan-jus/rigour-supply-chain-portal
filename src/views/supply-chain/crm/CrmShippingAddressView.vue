  ...
  <div class="crm-shipping-address-page supply-page supply-page--business-main">
    <div class="page-heading">
      <div>
        ...
        <SupplyPageTitle>客户地址</SupplyPageTitle>
        <p>查看客户收货地址簿，地址来源于 CRM 本地表，不在页面实时访问订货宝。</p>
      </div>
      <div class="heading-actions">
        <DhbPageSyncButton scope="ADDRESS" label="收货地址" @completed="loadRows" />
      ...
      <div class="table-viewport">
        ...
        >
          ...
          <el-table-column prop="consignee" label="收货人" min-width="130" show-overflow-tooltip>
            <template #default="scope">{{
              scope.row.consignee || scope.row.contact || '-'
            }}</template>
          </el-table-column>
          ...
          <el-table-column label="同步时间" width="180">
            <template #default="scope">{{
              formatTime(scope.row.syncedAt || scope.row.sourceUpdatedAt)
            }}</template>
          </el-table-column>
          <!-- @vue-generic {ShippingAddressSummaryView} -->
          <el-table-column label="操作" width="120" fixed="right" align="center">
            <template #default="scope">
              <el-button link type="primary" @click.stop="openCustomer(scope.row)"
                >查看客户</el-button
              >
            </template>
          </el-table-column>
...
const pageData = ref<CrmPage<ShippingAddressSummaryView>>({
  total: 0,
  begin: 0,
  step: 20,
  items: [],
...
function addressLabel(row: ShippingAddressSummaryView) {
  return (
    row.fullAddress ||
    [row.regionText, row.areaName, row.addressDetail].filter(Boolean).join('') ||
    '-'
  )
}
